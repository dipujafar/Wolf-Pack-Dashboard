...
import { useGetUserChartQuery } from "@/redux/api/metaApi";
...
export const UserOverview: React.FC = () => {
  ...
  const { data } = useGetUserChartQuery([{ label: "year", value: String(selectedYear) }]);
  const userChartData = data?.data || [];

  const formattedData = userChartData.map((item: any) => ({
    month: (item.month as string).substring(0, 3),
    count: item.count,
  }));

  return (
    <Card className='p-3 w-full bg-[#F9F9FA]/15 border-none'>
