...
const entrySchema = z.object({
  ...
  rank: z.number().min(1).max(3), // only 1–3 allowed
});
...
export default function PrizeForm() {
    ...
    if (prizes?.length) {
      form.reset({
        prizes: prizes.map((p: any) => ({
          ...
          isActive: p.isActive,
          entries: p.entries || [], // ensure entries are included
          file: undefined,
        })),
      });
  ...
  const updatePrize = async (prizeId: string, index: number) => {
    ...
    const payload = {
      ...
      tierLevel: prize.tierLevel,
      entries: prize.entries, // ✅ include entries in backend payload
    };
                ...
                <Card key={prize.id} className='bg-gray-800 border-gray-700'>
                  ...
                  <CardContent className='grid grid-cols-1 md:grid-cols-2 gap-6'>
                    ...
                    <div className='col-span-2 space-y-4'>
                      <Label className='text-gray-300'>Entries (Max 3)</Label>
                      {entryFields.map((entry, eIndex) => (
                        <div key={entry.id} className='flex space-x-2 items-center'>
                          <FormField
                            control={form.control}
                            name={`prizes.${index}.entries.${eIndex}.name`}
                            render={({ field }) => (
                              <FormItem className='flex-1'>
                                <FormLabel className='text-gray-300'>Entry Name</FormLabel>
                                <FormControl>
                                  <Input
                                    {...field}
                                    placeholder={`Entry ${eIndex + 1}`}
                                    className='bg-gray-700 border-gray-600 text-white'
                                  />
                                </FormControl>
                                <FormMessage />
                          ...
                          <FormField
                            control={form.control}
                            name={`prizes.${index}.entries.${eIndex}.rank`}
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel className='text-gray-300'>Rank</FormLabel>
                                <FormControl>
                                  <Input
                                    type='number'
                                    min={1}
                      ...
                      {entryFields.length < 3 && (
                        <Button
                          type='button'
                          size='sm'
                          onClick={() => appendEntry({ name: "", rank: entryFields.length + 1 })}
